pub use wit_bindgen_wasmi_impl::{export, import};
...
pub mod rt {
    ...
    pub fn char_from_i32(val: i32) -> Result<char, Trap> {
        char::from_u32(val as u32).ok_or_else(|| Trap::new("char value out of valid range"))
    }
    ...
    pub fn get_func<T>(caller: &mut Caller<'_, T>, func: &str) -> Result<Func, Trap> {
        let func = caller
            .get_export(func)
            .ok_or_else(|| {
    ...
    pub fn get_memory<T>(caller: &mut Caller<'_, T>, mem: &str) -> Result<Memory, Trap> {
        let mem = caller
            .get_export(mem)
            .ok_or_else(|| {
