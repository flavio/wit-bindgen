...
impl Wasmi {
    ...
    fn print_intrinsics(&mut self) {
        if self.needs_raw_mem {
            self.push_str("use wit_bindgen_wasmi::rt::RawMem;\n");
        }
        if self.needs_char_from_i32 {
            self.push_str("use wit_bindgen_wasmi::rt::char_from_i32;\n");
        }
        if self.needs_invalid_variant {
            self.push_str("use wit_bindgen_wasmi::rt::invalid_variant;\n");
        }
        if self.needs_bad_int {
            self.push_str("use core::convert::TryFrom;\n");
            self.push_str("use wit_bindgen_wasmi::rt::bad_int;\n");
        }
        if self.needs_validate_flags {
            self.push_str("use wit_bindgen_wasmi::rt::validate_flags;\n");
        }
        if self.needs_le {
            self.push_str("use wit_bindgen_wasmi::Le;\n");
        }
        if self.needs_copy_slice {
            self.push_str("use wit_bindgen_wasmi::rt::copy_slice;\n");
        }
    }
...
impl Generator for Wasmi {
    fn preprocess_one(&mut self, iface: &Interface, dir: Direction) {
        ...
        self.src
            .push_str("#[allow(unused_imports)]\nuse wit_bindgen_wasmi::{wasmi, anyhow};\n");
    ...
    fn type_record(
        ...
        {
            self.src.push_str("impl wit_bindgen_wasmi::Endian for ");
            ...
            // predicate).
            self.src
                .push_str("unsafe impl wit_bindgen_wasmi::AllBytesValid for ");
            self.src.push_str(&name.to_camel_case());
            self.src.push_str(" {}\n");
        }
    }
    ...
    fn type_flags(
        ...
        self.src
            .push_str("wit_bindgen_wasmi::bitflags::bitflags! {\n");
        self.rustdoc(docs);
    ...
    fn type_resource(&mut self, iface: &Interface, ty: ResourceId) {
        ...
        self.src.push_str(&format!(
            "pub struct {}(wit_bindgen_wasmi::rt::ResourceIndex);\n",
            tyname
        ));
    }
    ...
    fn export(&mut self, iface: &Interface, func: &Function) {
        ...
        // Generate the closure that's passed to a `Linker`, the final piece of
        // codegen here.
        self.src.push_str("move |mut caller: wasmi::Caller<'_, T>");
        ...
        if self.opts.tracing {
            self.src.push_str(&format!(
                "
                    let span = wit_bindgen_wasmi::tracing::span!(
                        wit_bindgen_wasmi::tracing::Level::TRACE,
                        \"wit-bindgen abi\",
                        module = \"{}\",
                        function = \"{}\",
                    );
        ...
        if needs_borrow_checker {
            self.src.push_str(
                ...
                let mut _bc = wit_bindgen_wasmi::BorrowChecker::new(mem);
                let host = get(data);\n",
            );
        } else {
    ...
    fn import(&mut self, iface: &Interface, func: &Function) {
        ...
        sig.async_ = is_async;
        sig.self_arg =
            Some("&self, mut caller: impl wasmi::AsContextMut<UserState = T>".to_string());
        self.print_docs_and_params(iface, func, TypeMode::AllBorrowed("'_"), &sig);
        self.push_str("-> Result<");
        self.print_ty(iface, &func.result, TypeMode::Owned);
        self.push_str(", wasmi::core::Trap> {\n");
    ...
    fn finish_one(&mut self, iface: &Interface, files: &mut Files) {
        for (module, funcs) in sorted_iter(&self.guest_imports) {
            ...
            let is_async = false;
            if is_async {
                self.src.push_str("#[wit_bindgen_wasmi::async_trait]\n");
            }
            ...
            if self.opts.custom_error {
                ...
                if self.needs_custom_error_to_trap {
                    self.src.push_str(
                        "fn error_to_trap(&mut self, err: Self::Error) -> wasmi::core::Trap;\n",
                    );
                }
                for ty in self.needs_custom_error_to_types.iter() {
                    self.src.push_str(&format!(
                        "fn error_to_{}(&mut self, err: Self::Error) -> Result<{}, wasmi::core::Trap>;\n",
                        ty.to_snake_case(),
                        ty.to_camel_case(),
                    ));
                }
            ...
            if self.all_needed_handles.len() > 0 {
                ...
                for handle in self.all_needed_handles.iter() {
                    ...
                    self.src.push_str("_table: wit_bindgen_wasmi::Table<T::");
                    self.src.push_str(&handle.to_camel_case());
                    self.src.push_str(">,\n");
                }
                self.src.push_str("}\n");
        ...
        for (module, funcs) in mem::take(&mut self.guest_imports) {
            ...
            if self.needs_get_memory {
                self.push_str("use wit_bindgen_wasmi::rt::get_memory;\n");
            }
                    ...
                    let resource_drop_{name}_fn = wasmi::Func::wrap(
                        ...
                        move |mut caller: wasmi::Caller<'_, T>, handle: u32| {{
                            ...
                            let handle = tables
                                ...
                                .remove(handle)
                                .map_err(|e| {{
                                    wasmi::core::Trap::new(format!(\"failed to remove handle: {{}}\", e))
                                }})?;
        ...
        for (module, exports) in sorted_iter(&mem::take(&mut self.guest_exports)) {
            ...
            for r in self.exported_resources.iter() {
                self.src.push_str(&format!(
                    "
                        index_slab{}: wit_bindgen_wasmi::rt::IndexSlab,
                        resource_slab{0}: wit_bindgen_wasmi::rt::ResourceSlab,
                        dtor{0}: Option<wasmi::TypedFunc<i32, ()>>,
                    ",
                    r.index()
                ));
            }
...
impl FunctionBindgen<'_> {
    fn new(gen: &mut Wasmi, params: Vec<String>) -> FunctionBindgen<'_> {
        FunctionBindgen {
            gen,
    ...
    fn emit(
        ...
        match inst {
            ...
            Instruction::I64FromU64 | Instruction::I64FromS64 => {
                ...
                results.push(format!("wit_bindgen_wasmi::rt::as_i64({})", s));
            }
            ...
            | Instruction::I32FromS32 => {
                let s = operands.pop().unwrap();
                results.push(format!("wit_bindgen_wasmi::rt::as_i32({})", s));
            }
            ...
            Instruction::HandleBorrowedFromI32 { ty } => {
                ...
                results.push(format!(
                    "_tables.{}_table.get(({}) as u32).ok_or_else(|| {{
                            wasmi::core::Trap::new(\"invalid handle index\")
                        }})?",
                    name.to_snake_case(),
                    operands[0]
                ));
            ...
            Instruction::CallInterface { module: _, func } => {
                ...
                if self.gen.opts.tracing && func.params.len() > 0 {
                    self.push_str("wit_bindgen_wasmi::tracing::event!(\n");
                    self.push_str("wit_bindgen_wasmi::tracing::Level::TRACE,\n");
                    for (i, (name, _ty)) in func.params.iter().enumerate() {
                        self.push_str(&format!(
                            "{} = wit_bindgen_wasmi::tracing::field::debug(&param{}),\n",
                            to_rust_ident(name),
                            i
                        ));
                    }
                ...
                match &func.result {
                    ...
                    _ if self.gen.opts.tracing => {
                        ...
                        self.push_str("wit_bindgen_wasmi::tracing::Level::TRACE,\n");
                        self.push_str(&format!(
                            "{} = wit_bindgen_wasmi::tracing::field::debug(&{0}),\n",
                            results[0],
                        ));
                        self.push_str(");\n");
                    }
...
impl NeededFunction {
    ...
    fn ty(&self) -> String {
        format!("wasmi::TypedFunc<{}>", self.cvt())
    }
}
