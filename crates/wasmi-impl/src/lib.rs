...
struct Opts {
    opts: wit_bindgen_gen_wasmi::Opts,
...
impl Parse for Opts {
    fn parse(input: ParseStream<'_>) -> Result<Opts> {
        ...
        let mut opts = wit_bindgen_gen_wasmi::Opts::default();
...
impl Parse for ConfigField {
    fn parse(input: ParseStream<'_>) -> Result<Self> {
        ...
        if l.peek(kw::src) {
            ...
            return Err(input.error("async support not supported by the `wit-bindgen-wasmi` crate"));
